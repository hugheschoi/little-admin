  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="标题图片" min-width="150px">
        <template slot-scope="{row}">
          <a class="link-type" :href="row.img" target="_blank"> 查看</a>
        </template>
      </el-table-column>
      <el-table-column label="内容链接" min-width="200px">
        <template slot-scope="{row}">
          <a class="link-type" :href="row.linkUrl" target="_blank"> 查看</a>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 450px; margin-left:50px;">
        ...
        <el-form-item label="内容链接" prop="linkUrl">
          <el-input v-model="temp.linkUrl" placeholder="请输入内容链接" />
        </el-form-item>
        <el-form-item label="标题图片" prop="img" style="margin-bottom: 30px;">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$ajax('/manager/explore/findAll', 'post', {
    ...
    handleDelete(row) {
      this.$ajax(`/manager/explore/delete`, 'post', {
        recData: {
          id: row.id
        }
      }).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$ajax('/manager/explore/add', 'post', {
            recData: this.temp
          }).then(res => {
            this.$message({
              showClose: true,
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.$ajax('/manager/explore/edit', 'post', {
            recData: tempData
          }).then(res => {
            this.$message({
              showClose: true,
