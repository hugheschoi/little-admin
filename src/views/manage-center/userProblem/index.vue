<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button round icon="el-icon-refresh" @click="initData">刷新</el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="反馈内容" min-width="250">
        <template slot-scope="{row}">
          <span class="text-over-flow link-type" :title="row.content" @click="checkDetail(row)">{{ row.content }}</span>
        </template>
      </el-table-column>
      <el-table-column label="反馈用户" width="100">
        <template slot-scope="{row}">
    ...
    <el-dialog custom-class="problemModal" title="反馈内容" :visible.sync="dialogTableVisible">
      <div>{{ content }}</div>
    </el-dialog>
    ...
    <!-- <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 450px; margin-left:50px;">
        ...
        <el-form-item label="协议内容" prop="content">
          <el-input type="textarea" maxlength="150" v-model="temp.content" placeholder="请输入标题" />
        </el-form-item>
        <el-form-item label="协议内容链接" prop="contentLink">
         ...
         <el-form-item label="协议类型" prop="type">
          <el-select v-model="temp.type" placeholder="请选择协议类型">
            ...
            <el-option label="隐私协议" value="privacy"></el-option>
            <el-option label="拍卖协议" value="auction"></el-option>
            <el-option label="保证金规则" value="deposit"></el-option>
            <el-option label="信用额度协议" value="credit"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$ajax('/manager/userProblem/findAll', 'post', {
    ...
    handleDelete(row) {
      this.$ajax(`/manager/userProblem/delete`, 'post', {
        recData: {
          id: row.id
        }
      }).then(response => {
